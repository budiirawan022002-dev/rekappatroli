...
function isLikelyCipopHeaderLabel($value)
{
    ...
    if (in_array($upper, $labels, true)) {
        return true;
    }

    // Label platform gabungan di baris header (mis. TWITTER|X.COM)
    if (strpos($upper, '|') !== false && preg_match('/(FACEBOOK|INSTAGRAM|TWITTER|TIKTOK|YOUTUBE|SNACKVIDEO|X\.COM)/', $upper)) {
        return true;
    }
...
/**
 * Temukan sheet berdasarkan nama (case-insensitive, trim spasi).
 */
function findSpreadsheetSheetByName($spreadsheet, $name)
{
    $needle = strtoupper(trim((string)$name));
    foreach ($spreadsheet->getSheetNames() as $sheetName) {
        if (strtoupper(trim((string)$sheetName)) === $needle) {
...
/**
 * Apakah nilai merupakan link postingan akun induk yang valid (bukan label header).
 */
function isValidAkunIndukPostLink($value)
{
    $value = trim((string)$value);
    if ($value === '' || isLikelyCipopHeaderLabel($value)) {
        return false;
    ...
    if (preg_match('#^https?://#i', $value)) {
        return (
            ...
            stripos($value, 'vt.tiktok.com') !== false ||
            stripos($value, 's.snackvideo.com') !== false
        );
    }

    return isLampiranUrlValue($value) && preg_match('#\.(com|net|org)/#i', $value);
}

/**
 * Apakah nilai merupakan nama akun induk yang valid (bukan label header/kolom).
 */
function isValidAkunIndukAccountName($value)
{
    $value = trim((string)$value);
    if ($value === '' || isLikelyCipopHeaderLabel($value)) {
        return false;
    }
    if (isLikelyCipopSequenceNumber($value) || isLikelyCipopMetricValue($value)) {
        return false;
    }
    if (isValidAkunIndukPostLink($value)) {
        return false;
    }

    $upper = strtoupper($value);
    $platformLabels = ['FACEBOOK', 'INSTAGRAM', 'TWITTER', 'X', 'X.COM', 'TIKTOK', 'YOUTUBE', 'SNACKVIDEO'];
    if (in_array($upper, $platformLabels, true)) {
        return false;
    }
...
/**
 * Ambil nilai link dari sel Excel (hyperlink + teks).
 */
function extractAkunIndukCellLink($cell)
{
    $linkTry = trim((string)extractExcelCellLinkValue($cell));
    if (isValidAkunIndukPostLink($linkTry)) {
        return $linkTry;
    }

    $scalar = trim((string)extractExcelCellScalarValue($cell));
    if (isValidAkunIndukPostLink($scalar)) {
        return $scalar;
    }
...
/**
 * Parse satu baris sheet AKUN INDUK — deteksi otomatis kolom nama, link, dan metrik.
 */
function parseAkunIndukRowFromSheet($sheet, $row)
{
    $cells = [];
    foreach (['A', 'B', 'C', 'D', 'E', 'F', 'G'] as $col) {
        $cell = $sheet->getCell($col . $row);
        if (in_array($col, ['B', 'C', 'D', 'E', 'F', 'G'], true)) {
            $linkVal = extractAkunIndukCellLink($cell);
            $cells[$col] = $linkVal !== '' ? $linkVal : trim((string)extractExcelCellScalarValue($cell));
        } else {
            $cells[$col] = trim((string)extractExcelCellScalarValue($cell));
        }
    }

    $link = '';
    $linkCol = null;
    foreach (['C', 'D', 'E', 'B', 'F', 'G'] as $col) {
        if (isValidAkunIndukPostLink($cells[$col] ?? '')) {
            $link = $cells[$col];
            $linkCol = $col;
            break;
        }
    ...
    $nama = '';
    foreach (['B', 'C', 'D', 'A'] as $col) {
        if ($col === $linkCol) {
            continue;
        }
        $value = trim((string)($cells[$col] ?? ''));
        if (isValidAkunIndukAccountName($value)) {
            $nama = $value;
            break;
        }
    }
    ...
    foreach (['D', 'E', 'F', 'G'] as $col) {
        ...
        if (isLikelyCipopMetricValue($value) || is_numeric(str_replace([',', ' '], '', $value))) {
            $metrics[] = $value;
        }
    }

    if ($nama === '' && $link !== '') {
        $nama = deriveAccountNameFromSocialLink($link);
    }
...
/**
 * Apakah baris berisi data akun induk valid (punya link postingan).
 */
function isValidAkunIndukParsedRow(array $parsed)
{
    return isValidAkunIndukPostLink($parsed['link_postingan'] ?? '');
}

/**
 * Cari baris data akun induk dalam blok platform (lewati baris header).
 */
function findAkunIndukDataRowNearPlatform($sheet, $platform, $defaultRow)
{
    $mapping = getAkunIndukSheetMapping();
    $platformKeys = array_keys($mapping);
    $platformIndex = array_search($platform, $platformKeys, true);
    $nextDefaultRow = ($platformIndex !== false && isset($platformKeys[$platformIndex + 1]))
        ? (int)$mapping[$platformKeys[$platformIndex + 1]]['row']
        : min((int)$sheet->getHighestRow(), $defaultRow + 6);

    $startRow = max(1, $defaultRow - 1);
    $endRow = min((int)$sheet->getHighestRow(), max($nextDefaultRow - 1, $defaultRow + 3));

    for ($row = $startRow; $row <= $endRow; $row++) {
        $parsed = parseAkunIndukRowFromSheet($sheet, $row);
        if (isValidAkunIndukParsedRow($parsed)) {
            return ['row' => $row, 'data' => $parsed];
        }
    }

    $parsed = parseAkunIndukRowFromSheet($sheet, $defaultRow);
    return ['row' => $defaultRow, 'data' => $parsed];
}

/**
 * Temukan baris data per platform di sheet AKUN INDUK (label platform → baris data di bawahnya).
 */
function discoverAkunIndukPlatformRows($sheet)
{
    $mapping = getAkunIndukSheetMapping();
    $discovered = [];
    $highest = min((int)$sheet->getHighestRow(), 60);

    for ($row = 1; $row <= $highest; $row++) {
        foreach (['A', 'B'] as $col) {
            $label = strtoupper(trim((string)extractExcelCellScalarValue($sheet->getCell($col . $row))));
            if ($label === '') {
                continue;
            }

            foreach (array_keys($mapping) as $platform) {
                if ($label !== $platform && strpos($label, $platform) === false) {
                    continue;
                }
                if (isset($discovered[$platform])) {
                    continue;
                }

                for ($scan = $row + 1; $scan <= min($row + 4, $highest); $scan++) {
                    $parsed = parseAkunIndukRowFromSheet($sheet, $scan);
                    if (isValidAkunIndukParsedRow($parsed)) {
                        $discovered[$platform] = $scan;
                        break;
                    }
                }
            }
...
function readAkunIndukFromUploadedExcel(array $files)
{
    ...
    for ($i = 0; $i < count($files['name']); $i++) {
        ...
        try {
            $spreadsheet = IOFactory::load($uploadedFile);
            $sheet = findSpreadsheetSheetByName($spreadsheet, 'AKUN INDUK');
            if (!$sheet) {
                error_log('readAkunIndukFromUploadedExcel: sheet AKUN INDUK tidak ditemukan pada ' . ($files['name'][$i] ?? ''));
                continue;
            }

            $discoveredRows = discoverAkunIndukPlatformRows($sheet);

            foreach ($mapping as $platform => $config) {
                $parsed = null;
                $dataRow = null;

                if (isset($discoveredRows[$platform])) {
                    $dataRow = $discoveredRows[$platform];
                    $parsed = parseAkunIndukRowFromSheet($sheet, $dataRow);
                }

                if (!$parsed || !isValidAkunIndukParsedRow($parsed)) {
                    $found = findAkunIndukDataRowNearPlatform($sheet, $platform, $config['row']);
                    $dataRow = $found['row'];
                    $parsed = $found['data'];
                }
                ...
                if (!isValidAkunIndukParsedRow($parsed)) {
                    foreach ($config['fields'] as $field => $col) {
                        $cell = $sheet->getCell($col . $config['row']);
                        if ($field === 'link_postingan') {
                            $parsed[$field] = extractAkunIndukCellLink($cell);
                        } elseif ($field === 'nama_akun') {
                            $value = trim((string)extractExcelCellScalarValue($cell));
                            $parsed[$field] = isValidAkunIndukAccountName($value) ? $value : '';
                        } else {
                            $parsed[$field] = trim((string)extractExcelCellScalarValue($cell));
                        }
                    }
                    if (($parsed['nama_akun'] ?? '') === '' && isValidAkunIndukPostLink($parsed['link_postingan'] ?? '')) {
                        $parsed['nama_akun'] = deriveAccountNameFromSocialLink($parsed['link_postingan']);
                    }
                }
                ...
                if (isValidAkunIndukParsedRow($parsed)) {
                    error_log("AKUN INDUK {$platform} (baris {$dataRow}): nama='{$parsed['nama_akun']}', link=" . substr($parsed['link_postingan'], 0, 80));
                } else {
                    error_log("AKUN INDUK {$platform}: data tidak valid — nama='{$parsed['nama_akun']}', link=" . substr((string)($parsed['link_postingan'] ?? ''), 0, 40));
                }
            }
...
/**
 * Format metrik engagement narasi WA akun induk — pakai nilai asli sheet AKUN INDUK Excel input.
 * Acak kecil hanya jika sel kosong.
 */
...
function buildAkunPendukungStatsPerPlatform($platform, array $dataLink, array $akunIndukRow = [])
{
    ...
    $indukLink = rtrim(strtolower(trim((string)($akunIndukRow['link_postingan'] ?? ''))), '/');
    ...
    foreach ($rows as $row) {
        ...
        $linkNorm = rtrim(strtolower($link), '/');
        if ($indukLink !== '' && ($linkNorm === $indukLink || strpos($linkNorm, $indukLink) !== false || strpos($indukLink, $linkNorm) !== false)) {
            continue;
        }
...
/**
 * Label metrik share/retweet untuk narasi WA Kasatgas MBG.
 */
function getWaAkunIndukShareLabel($platformKey)
{
    return in_array(strtoupper($platformKey), ['TWITTER', 'YOUTUBE'], true) ? 'Retweets' : 'Share';
}

/**
 * Bangun section AMPLIFIKASI AKUN INDUK (data sheet AKUN INDUK Excel input).
 */
function buildNarasiAmplifikasiAkunIndukSection(array $akunIndukData)
{
    $sectionInduk = "*AMPLIFIKASI AKUN INDUK*\n";
    $idxInduk = 1;
    ...
    foreach ($indukPlatforms as $plat) {
        $induk = $akunIndukData[$plat['key']] ?? [];
        $nama = trim((string)($induk['nama_akun'] ?? ''));
        $link = trim((string)($induk['link_postingan'] ?? ''));

        if (!isValidAkunIndukPostLink($link)) {
            continue;
        }

        if (!isValidAkunIndukAccountName($nama)) {
            $nama = deriveAccountNameFromSocialLink($link);
        }
        if ($nama === '') {
            $nama = 'Akun ' . $plat['wa'];
        ...
        $like = formatWaEngagementMetric($induk['like'] ?? '', 1, 12);
        $comments = formatWaEngagementMetric($induk['comments'] ?? '', 0, 12);
        $share = formatWaEngagementMetric($induk['share'] ?? '', 0, 12);
        $views = formatWaEngagementMetric($induk['views'] ?? '', 2, 15);
        $shareLabel = getWaAkunIndukShareLabel($plat['key']);

        $linkPart = $link !== '' ? " ({$link})" : '';
        $sectionInduk .= "{$idxInduk}. Akun {$plat['wa']} *{$nama}*{$linkPart}, dengan engagement:\n";
        $sectionInduk .= "* {$like} Like\n";
        $sectionInduk .= "* {$comments} Comments\n";
        $sectionInduk .= "* {$share} {$shareLabel}\n";
        $sectionInduk .= "* {$views} View\n\n";
        $idxInduk++;
    }

    if ($idxInduk === 1) {
        $sectionInduk .= "(Data akun induk tidak ditemukan pada sheet AKUN INDUK file Excel)\n\n";
    }
...
/**
 * Bangun section AMPLIFIKASI AKUN PENDUKUNG.
 */
function buildNarasiAmplifikasiAkunPendukungSection(array $dataLink, array $akunIndukData)
{
    $sectionPendukung = "*AMPLIFIKASI AKUN PENDUKUNG*\n";
    $idxPendukung = 1;
    ...
    foreach ($pendukungLabels as $key => $label) {
        ...
        $sectionPendukung .= "{$idxPendukung}. *{$label}*\n";
        $sectionPendukung .= "* {$pendukung['akun']} Jumlah Akun\n";
        $sectionPendukung .= "* {$pendukung['posting']} Jumlah Postingan\n\n";
        $idxPendukung++;
    }

    if ($idxPendukung === 1) {
        $sectionPendukung .= "(Tidak ada data akun pendukung pada file Excel)\n\n";
    }

    return $sectionPendukung;
}

/**
 * Narasi WA Kasatgas MBG (patroli + amplifikasi) — terpisah dari narasi Rajawali/Elang/Kasuari.
 */
function buildNarasiWaAmplifikasiMbgKhusus(
    ...
    $sectionInduk = buildNarasiAmplifikasiAkunIndukSection($akunIndukData);
    $sectionPendukung = buildNarasiAmplifikasiAkunPendukungSection($dataLink, $akunIndukData);
    $sectionAmplifikasi = "*C. AMPLIFIKASI DIGITAL*\n\n{$sectionInduk}{$sectionPendukung}";

    return <<<EOD
*Kepada : Yth. Kasatgas MBG*

*Dari : Katim Wilayah Jambi*

*Tembusan :*
*Yth. 1. Kabag Ops MBG*
*2. Kabag Anev MBG*

*Perihal : Laporan Temuan Akun Medsos Narasi Negatif MBG dan Amplifikasi Dukungan Program MBG di Wilayah Prov. Jambi Update {$tanggalFormattedFirst}*
...
{$isiPatroliMbg}{$sectionAmplifikasi}*D. CATATAN*
...
/**
 * Bangun narasi amplifikasi (section C) — ringkasan per platform + akun induk & pendukung.
 */
function buildNarasiAmplifikasiMbg($dataLink, $jumlahAkunperSheet, $jumlahLinkperSheet, array $akunIndukData = [])
{
    $platforms = [
    ...
    $output .= "\n";
    $output .= buildNarasiAmplifikasiAkunIndukSection($akunIndukData);
    $output .= buildNarasiAmplifikasiAkunPendukungSection($dataLink, $akunIndukData);
...
/**
 * Generate engagement acak kecil untuk file Excel lampiran amplifikasi MBG (bukan akun induk WA).
 */
function generateRandomLampiranEngagement($platform = '')
{
    ...
    $ranges = [
        'FACEBOOK'   => ['views' => [2, 15], 'like' => [1, 12], 'comments' => [0, 12], 'shares' => [0, 12]],
        'INSTAGRAM'  => ['views' => [2, 15], 'like' => [1, 12], 'comments' => [0, 12], 'shares' => [0, 12]],
        'TWITTER'    => ['views' => [2, 15], 'like' => [1, 12], 'comments' => [0, 12], 'shares' => [0, 12]],
        'TIKTOK'     => ['views' => [2, 15], 'like' => [1, 12], 'comments' => [0, 12], 'shares' => [0, 12]],
        'YOUTUBE'    => ['views' => [2, 15], 'like' => [1, 12], 'comments' => [0, 12], 'shares' => [0, 12]],
        'SNACKVIDEO' => ['views' => [2, 15], 'like' => [1, 12], 'comments' => [0, 12], 'shares' => [0, 12]],
    ];

    $r = $ranges[$platform] ?? ['views' => [2, 15], 'like' => [1, 12], 'comments' => [0, 12], 'shares' => [0, 12]];
...
/**
 * Escape teks untuk sel a:t di dalam slide PPTX.
 */
function pptxEscapeText($text)
{
    return htmlspecialchars((string)$text, ENT_XML1 | ENT_QUOTES, 'UTF-8');
}

/**
 * Gabungkan semua teks dari slide PPTX.
 */
function pptxExtractSlidePlainText($slideXml)
{
    preg_match_all('/<a:t>([^<]*)<\/a:t>/', (string)$slideXml, $matches);

    return implode('', $matches[1] ?? []);
}

/**
 * Format metrik engagement untuk slide PPT akun induk (nilai asli Excel).
 */
function formatPptEngagementMetric($value, $min = 1, $max = 12)
{
    $normalized = normalizeLampiranMetricValue($value);
    if ($normalized === '') {
        return random_int($min, $max);
...
/**
 * Satu baris teks akun induk untuk slide PPT (format template).
 */
function formatPptSingleAkunIndukLine($index, $platformWa, $platformKey, array $indukRow)
{
    $nama = trim((string)($indukRow['nama_akun'] ?? ''));
    $link = trim((string)($indukRow['link_postingan'] ?? ''));
    ...
    if ($nama === '') {
        $nama = 'Akun ' . $platformWa;
    }

    $like = formatPptEngagementMetric($indukRow['like'] ?? '', 1, 500);
    $comments = formatPptEngagementMetric($indukRow['comments'] ?? '', 0, 100);
    $share = formatPptEngagementMetric($indukRow['share'] ?? '', 0, 250);
    $views = formatPptEngagementMetric($indukRow['views'] ?? '', 2, 50000);
    $shareLabel = getWaAkunIndukShareLabel($platformKey);

    return "{$index}. Akun {$platformWa} *{$nama}* ({$link}), dengan engagement:"
        . "* {$like} Like"
        . "* {$comments} Comments"
        . "* {$share} {$shareLabel}"
        . "* {$views} View";
...
/**
 * Blok teks akun induk untuk satu slide PPT (2 platform).
 */
function buildPptAkunIndukSlideBlock(array $platformConfigs, array $akunIndukData, &$startIndex = 1)
{
    $parts = [];
    foreach ($platformConfigs as $cfg) {
        $line = formatPptSingleAkunIndukLine(
            $startIndex,
            $cfg['wa'],
            $cfg['key'],
            $akunIndukData[$cfg['key']] ?? []
        );
        if ($line === '') {
            continue;
        }
...
/**
 * Normalisasi tanda kutip untuk pencocokan teks PPT.
 */
function normalizePptxMatchText($text)
{
    $text = (string)$text;
    $replacements = [
        "\u{201C}" => '"', "\u{201D}" => '"',
...
/**
 * Ganti teks slide PPTX — coba exact match, lalu versi normalisasi kutip.
 */
function pptxReplaceSlideXmlText($slideXml, $oldText, $newText)
{
    if ($oldText === '' || $newText === '') {
        return $slideXml;
    }

    if (strpos($slideXml, $oldText) !== false) {
        return str_replace($oldText, pptxEscapeText($newText), $slideXml);
    }

    $oldNorm = normalizePptxMatchText($oldText);
    if ($oldNorm !== '' && $newText !== '' && strpos($slideXml, $oldNorm) !== false) {
        return str_replace($oldNorm, pptxEscapeText($newText), $slideXml);
    }

    // Fallback: cari di plain text lalu ganti substring pertama yang cocok di XML
    $plain = pptxExtractSlidePlainText($slideXml);
    $plainNorm = normalizePptxMatchText($plain);
    $oldInPlain = normalizePptxMatchText($oldText);

    if ($oldInPlain !== '' && strpos($plainNorm, $oldInPlain) !== false) {
        $pos = strpos($plainNorm, $oldInPlain);
        $candidate = substr($plain, $pos, strlen($oldInPlain));
        if ($candidate !== '' && strpos($slideXml, $candidate) !== false) {
            return str_replace($candidate, pptxEscapeText($newText), $slideXml);
        }
    }
...
/**
 * Ganti teks yang terpecah di beberapa run a:t (PowerPoint sering split per kata/frasa).
 */
function pptxReplaceTextAcrossRuns($slideXml, $oldText, $newText)
{
    if ($oldText === '' || $newText === '') {
        return $slideXml;
    }

    $replaced = pptxReplaceSlideXmlText($slideXml, $oldText, $newText);
    if ($replaced !== $slideXml) {
        return $replaced;
    }

    if (!preg_match_all('/<a:t([^>]*)>([^<]*)<\/a:t>/', $slideXml, $matches, PREG_OFFSET_CAPTURE)) {
        return $slideXml;
    }
    ...
    $oldNorm = normalizePptxMatchText($oldText);
    $cumNorm = normalizePptxMatchText($cumulative);
    $pos = strpos($cumulative, $oldText);
    $matchLen = strlen($oldText);

    if ($pos === false && $oldNorm !== '') {
        $pos = strpos($cumNorm, $oldNorm);
        $matchLen = strlen($oldNorm);
    }
    ...
    foreach ($runs as $idx => $run) {
        $runLen = strlen($run[0]);
        if ($startRun === null && ($cumLen + $runLen) > $pos) {
            $startRun = $idx;
        }
        if ($startRun !== null && ($cumLen + $runLen) >= $endPos) {
            $endRun = $idx;
            break;
        }
        $cumLen += $runLen;
    ...
    $escapedNew = pptxEscapeText($newText);
    $result = $slideXml;

    for ($i = $startRun; $i <= $endRun; $i++) {
        $fullTag = '<a:t' . $matches[1][$i][0] . '>' . $runs[$i][0] . '</a:t>';
        $replacement = ($i === $startRun)
            ? '<a:t' . $matches[1][$i][0] . '>' . $escapedNew . '</a:t>'
            : '<a:t' . $matches[1][$i][0] . '></a:t>';
        $result = str_replace($fullTag, $replacement, $result);
    }
...
/**
 * Perbaiki judul slide gambar (KONTEN / SCREENSHOOT) agar tidak wrap dan menutupi foto.
 */
function pptxFixMediaSlideHeading($slideXml, $headingText)
{
    if ($headingText === '' || strpos($slideXml, '<a:t>' . $headingText . '</a:t>') === false) {
        return $slideXml;
    }
    ...
    $minCx = $minWidthEmu[$headingText] ?? 5200000;
    $minCy = 1100000;

    $pattern = '/(<p:sp>(?:(?!<\/p:sp>).)*?<a:t>' . preg_quote($headingText, '/') . '<\/a:t>(?:(?!<\/p:sp>).)*?<\/p:sp>)/s';

    $fixed = preg_replace_callback(
        $pattern,
        static function ($match) use ($minCx, $minCy) {
            $shape = $match[1];

            $shape = preg_replace(
                '/<a:bodyPr([^>]*)>\s*<a:spAutoFit\s*\/>\s*<\/a:bodyPr>/',
                '<a:bodyPr$1 wrap="none"/>',
                $shape
            );

            $shape = preg_replace_callback(
                '/<a:ext cx="(\d+)" cy="(\d+)"/',
                static function ($extMatch) use ($minCx, $minCy) {
                    $cx = max((int)$extMatch[1], $minCx);
                    $cy = max((int)$extMatch[2], $minCy);
    ...
    return is_string($fixed) ? $fixed : $slideXml;
}

/**
 * Ganti isi body slide setelah marker header (fleksibel jika template berubah sedikit).
 */
function pptxReplaceSlideBodyAfterMarker($slideXml, $plainText, $startMarker, $newBody)
{
    $plainNorm = normalizePptxMatchText($plainText);
    $markerNorm = normalizePptxMatchText($startMarker);
    $pos = strpos($plainNorm, $markerNorm);

    if ($pos === false) {
        return $slideXml;
    }

    $oldBody = substr($plainText, $pos);
    if ($oldBody === '') {
        return $slideXml;
    }

    $replaced = pptxReplaceSlideXmlText($slideXml, $oldBody, $newBody);
    if ($replaced !== $slideXml) {
        return $replaced;
    }

    // Regex fallback: dari marker sampai akhir blok teks akun induk
    if (preg_match('/' . preg_quote($startMarker, '/') . '.+/s', normalizePptxMatchText($plainText), $m)) {
        return pptxReplaceSlideXmlText($slideXml, $m[0], $newBody);
    }
...
/**
 * Daftar platform akun induk yang punya data valid dari Excel.
 */
function getValidPptAkunIndukPlatforms(array $akunIndukData)
{
    $order = [
        ['key' => 'FACEBOOK', 'wa' => 'Facebook'],
        ['key' => 'INSTAGRAM', 'wa' => 'Instagram'],
    ...
    $valid = [];
    foreach ($order as $plat) {
        $row = $akunIndukData[$plat['key']] ?? [];
        if (isValidAkunIndukPostLink($row['link_postingan'] ?? '')) {
            $valid[] = $plat;
        }
    }
...
/**
 * Kelompokkan platform akun induk per slide (default 2 platform/slide).
 */
function chunkPptAkunIndukPlatforms(array $platforms, $perSlide = 2)
{
    if (empty($platforms)) {
        return [];
    }

    return array_chunk($platforms, max(1, (int)$perSlide));
}

/**
 * Marker awal blok akun induk di template per slide (slide 3,4,5).
 */
function getPptAkunIndukSlideStartMarkers()
{
    return [
        3 => '1. Akun Facebook',
        4 => '3. Akun Twitter',
...
/**
 * Paragraf ringkasan amplifikasi untuk slide 2 PPT — total akun & postingan dari sheet Excel.
 */
function buildPptCipopMbgSummaryParagraph(array $jumlahAkunperSheet, array $jumlahLinkperSheet)
{
    $items = [
        ['key' => 'FACEBOOK', 'label' => 'Facebook'],
        ['key' => 'TWITTER', 'label' => 'Twitter/X', 'altKeys' => ['X']],
    ...
    foreach ($items as $item) {
        ...
        foreach ($item['altKeys'] ?? [] as $altKey) {
            if ($akun <= 0) {
                $akun = (int)($jumlahAkunperSheet[$altKey] ?? 0);
            }
            if ($link <= 0) {
                $link = (int)($jumlahLinkperSheet[$altKey] ?? 0);
            }
        }
        ...
        $parts[] = "{$item['label']} sebanyak {$akun} akun ({$link} postingan)";
    }

    if (empty($parts)) {
        return 'Telah dilakukan upaya amplifikasi konten "Makan Bergizi Gratis (MBG)" di Wilayah Merpati-14.';
    }

    $last = array_pop($parts);
    $detail = count($parts) > 0 ? implode(', ', $parts) . ', serta ' . $last : $last;

    return 'Telah dilakukan upaya amplifikasi konten "Makan Bergizi Gratis (MBG)" di Wilayah Merpati-14 dengan total ' . $detail . '.';
}

/**
 * Teks tanggal cover slide 1 PPT (contoh: KAMIS, 24 JUNI 2026).
 */
function buildPptCipopMbgCoverDateText($tanggalFormattedFirst, $hariFormatted)
{
    $parts = preg_split('/\s+/', trim((string)$tanggalFormattedFirst));
    $day = $parts[0] ?? '';
    $year = $parts[count($parts) - 1] ?? '';
    $month = count($parts) >= 3 ? implode(' ', array_slice($parts, 1, -1)) : '';

    $hari = strtoupper(trim((string)$hariFormatted));
    $monthUpper = strtoupper(trim((string)$month));

    return trim($hari . ', ' . $day . ' ' . $monthUpper . ' ' . $year);
}

/**
 * Ganti semua run teks (a:t) setelah marker tertentu dalam satu slide XML.
 */
function pptxReplaceAllTextAfterMarker($slideXml, $marker, $newText)
{
    if ($marker === '' || $newText === '') {
        return $slideXml;
    }

    $markerNorm = normalizePptxMatchText($marker);
    $plain = pptxExtractSlidePlainText($slideXml);
    $plainNorm = normalizePptxMatchText($plain);
    $pos = strpos($plainNorm, $markerNorm);
    ...
    foreach ($runs as $idx => $run) {
        $cumulative .= $run[0];
        if ($startRun === null && strpos(normalizePptxMatchText($cumulative), $markerNorm) !== false) {
            $startRun = $idx;
        }
    }
    ...
    $firstOffset = $runs[$startRun][1];
    $txBodyStart = strrpos(substr($slideXml, 0, $firstOffset), '<p:txBody');
    $txBodyEnd = strpos($slideXml, '</p:txBody>', $firstOffset);
    if ($txBodyEnd === false) {
        $txBodyEnd = strlen($slideXml);
    }

    for ($i = $startRun; $i < count($runs); $i++) {
        if ($runs[$i][1] < $txBodyStart || $runs[$i][1] > $txBodyEnd) {
            $endRun = max($startRun, $i - 1);
            break;
        }
        $endRun = $i;
    }
...
/**
 * Cari paragraf ringkasan lama di slide 2 template.
 */
function extractPptTemplateSummaryParagraph($plainText)
{
    if (preg_match('/Telah dilakukan upaya amplifikasi konten.+?\./us', $plainText, $m)) {
        return $m[0];
    }
...
/**
 * Ambil blok teks akun induk lama dari plain text slide template.
 */
function extractPptTemplateAkunBlock($plainText, $startsWith)
{
    $pos = strpos($plainText, $startsWith);
    if ($pos === false) {
        return '';
    }

    return substr($plainText, $pos);
}

/**
 * Siapkan daftar gambar untuk slot media PPT (konten + screenshoot).
 */
function buildPptCipopMbgImageList(array $imagePaths, $isScreenshot = false)
{
    $distribution = distributeCipopMbgImages($imagePaths, $isScreenshot);
    $konten = array_values($distribution['konten'] ?? []);
    $screens = array_values($distribution['screenshots'] ?? []);

    if (empty($screens) && !empty($distribution['laporan'])) {
        $screens = array_values($distribution['laporan']);
    }

    while (count($konten) < 3) {
        $konten[] = !empty($konten) ? end($konten) : null;
    }
    while (count($screens) < 6) {
        $screens[] = !empty($screens) ? end($screens) : null;
    }

    return array_merge(array_slice($konten, 0, 3), array_slice($screens, 0, 6));
}

/**
 * Konversi PPTX ke PDF via Microsoft PowerPoint COM (Windows).
 */
function convertPptxToPdfUsingPowerPoint($pptxPath, $pdfPath)
{
    if (!file_exists($pptxPath)) {
        return false;
    }

    $pptxPath = str_replace('/', '\\', realpath($pptxPath) ?: $pptxPath);
...
try {
    ...
    if ($app -ne $null) { $app.Quit(); [void][System.Runtime.Interopservices.Marshal]::ReleaseComObject($app) }
}
PS1;

    $tmpScript = sys_get_temp_dir() . DIRECTORY_SEPARATOR . 'convert_pptx_pdf_' . uniqid('', true) . '.ps1';
    file_put_contents($tmpScript, $script);

    $command = 'powershell -NoProfile -ExecutionPolicy Bypass -File '
        . escapeshellarg($tmpScript)
        . ' -InputPath ' . escapeshellarg($pptxPath)
        . ' -OutputPath ' . escapeshellarg($pdfPath);

    exec($command, $output, $exitCode);
    @unlink($tmpScript);

    if ($exitCode === 0 && is_file($pdfPath) && filesize($pdfPath) > 0) {
        error_log('PPT->PDF berhasil via PowerPoint COM: ' . $pdfPath);
        return true;
    }

    error_log('PPT->PDF gagal via PowerPoint COM. Exit code: ' . $exitCode . ' Output: ' . implode(' | ', $output));
    return false;
}

/**
 * Konversi PPTX ke PDF via LibreOffice (fallback).
 */
function convertPptxToPdfUsingLibreOffice($pptxPath, $pdfPath)
{
    $candidates = [
    ...
    foreach ($candidates as $soffice) {
        ...
        $cmd = escapeshellarg($soffice)
            ...
            . escapeshellarg($pptxReal);

        exec($cmd, $output, $exitCode);
        $generated = $outDir . DIRECTORY_SEPARATOR . pathinfo($pptxReal, PATHINFO_FILENAME) . '.pdf';
        if ($exitCode === 0 && is_file($generated)) {
            if (realpath($generated) !== realpath($pdfPath)) {
                @rename($generated, $pdfPath);
            }
            if (is_file($pdfPath) && filesize($pdfPath) > 0) {
                error_log('PPT->PDF berhasil via LibreOffice: ' . $pdfPath);
                return true;
            }
        }
    }
...
/**
 * Isi template PPT amplifikasi Cipop MBG dan simpan sebagai PPTX.
 */
function createCipopMbgAmplifikasiPptxFromTemplate(
    $templatePath,
    $outputPptxPath,
    array $imagePaths,
    $tanggalFormattedFirst,
    ...
    if (!file_exists($templatePath)) {
        throw new Exception('Template PPT amplifikasi tidak ditemukan: ' . $templatePath);
    }

    if (!@copy($templatePath, $outputPptxPath)) {
        throw new Exception('Gagal menyalin template PPT amplifikasi.');
    }

    $zip = new ZipArchive();
    if ($zip->open($outputPptxPath) !== true) {
        throw new Exception('Gagal membuka file PPT hasil.');
    }
    ...
    if ($templateZip->open($templatePath) !== true) {
        $zip->close();
        throw new Exception('Gagal membaca template PPT amplifikasi.');
    }

    $slide1Tpl = $templateZip->getFromName('ppt/slides/slide1.xml');
    $slide2Tpl = $templateZip->getFromName('ppt/slides/slide2.xml');
    $slide3Tpl = $templateZip->getFromName('ppt/slides/slide3.xml');
    $slide4Tpl = $templateZip->getFromName('ppt/slides/slide4.xml');
    $slide5Tpl = $templateZip->getFromName('ppt/slides/slide5.xml');
    $templateZip->close();

    $plain1 = pptxExtractSlidePlainText($slide1Tpl);
    $plain2 = pptxExtractSlidePlainText($slide2Tpl);
    $plain3 = pptxExtractSlidePlainText($slide3Tpl);
    $plain4 = pptxExtractSlidePlainText($slide4Tpl);
    $plain5 = pptxExtractSlidePlainText($slide5Tpl);

    if (preg_match('/[A-Z]+,\s*\d+\s+[A-Z]+\s+\d{4}/', $plain1, $mDateRaw)) {
        $newDate = buildPptCipopMbgCoverDateText($tanggalFormattedFirst, $hariFormatted);
        $slide1 = pptxReplaceTextAcrossRuns($slide1Tpl, $mDateRaw[0], $newDate);
        if ($slide1 === $slide1Tpl) {
            $slide1 = pptxReplaceSlideXmlText($slide1Tpl, $mDateRaw[0], $newDate);
        }
        $zip->addFromString('ppt/slides/slide1.xml', $slide1);
        error_log('PPT slide1 date: ' . $newDate);
    }

    $oldSummary = extractPptTemplateSummaryParagraph($plain2);
    $newSummary = buildPptCipopMbgSummaryParagraph($jumlahAkunperSheet, $jumlahLinkperSheet);
    if ($oldSummary !== '') {
        $slide2 = pptxReplaceAllTextAfterMarker($slide2Tpl, 'Telah dilakukan', $newSummary);
        if ($slide2 === $slide2Tpl) {
            $slide2 = pptxReplaceSlideXmlText($slide2Tpl, $oldSummary, $newSummary);
        }
        $zip->addFromString('ppt/slides/slide2.xml', $slide2);
        error_log('PPT slide2 summary: ' . substr($newSummary, 0, 120) . '...');
    }

    $validPlatforms = getValidPptAkunIndukPlatforms($akunIndukData);
    $platformChunks = chunkPptAkunIndukPlatforms($validPlatforms, 2);
    $slideMarkers = getPptAkunIndukSlideStartMarkers();
    $slideTemplates = [3 => $slide3Tpl, 4 => $slide4Tpl, 5 => $slide5Tpl];
    $slidePlains = [3 => $plain3, 4 => $plain4, 5 => $plain5];
    $akunIndex = 1;

    foreach ($slideMarkers as $slideNum => $defaultMarker) {
        if (!isset($slideTemplates[$slideNum])) {
            continue;
        }

        $chunkIndex = (int)$slideNum - 3;
        $platformGroup = $platformChunks[$chunkIndex] ?? [];

        $newBlock = !empty($platformGroup)
            ? buildPptAkunIndukSlideBlock($platformGroup, $akunIndukData, $akunIndex)
            : '(Data akun induk tidak tersedia pada sheet AKUN INDUK untuk slide ini)';

        $plain = $slidePlains[$slideNum];
        $tpl = $slideTemplates[$slideNum];
        $marker = $defaultMarker;

        // Cari marker dinamis: "N. Akun Platform" pertama di slide
        if (preg_match('/\d+\. Akun (Facebook|Instagram|Twitter|Tiktok|Youtube|SnackVideo)/', $plain, $mMarker)) {
            $marker = $mMarker[0];
        }

        $oldBlock = extractPptTemplateAkunBlock($plain, $marker);
        if ($oldBlock === '' && preg_match('/\d+\. Akun .+/', $plain, $mBlock)) {
            $oldBlock = extractPptTemplateAkunBlock($plain, $mBlock[0]);
        }

        if ($oldBlock !== '' || $marker !== '') {
            $slideXml = pptxReplaceAllTextAfterMarker($tpl, $marker, $newBlock);
            if ($slideXml === $tpl && $oldBlock !== '') {
                $slideXml = pptxReplaceSlideXmlText($tpl, $oldBlock, $newBlock);
            }
            if ($slideXml === $tpl) {
                $slideXml = pptxReplaceSlideBodyAfterMarker($tpl, $plain, $marker, $newBlock);
            }
            $zip->addFromString('ppt/slides/slide' . $slideNum . '.xml', $slideXml);
            error_log("PPT slide{$slideNum} akun induk diperbarui (" . count($platformGroup) . ' platform)');
        }
    }
    ...
    $images = buildPptCipopMbgImageList($imagePaths, $isScreenshot);
    $tmpImages = [];
    foreach ($mediaTargets as $idx => $mediaPath) {
        if ($zip->locateName($mediaPath) === false) {
            continue;
        }

        $source = $images[$idx] ?? null;
        if (!$source || !is_file($source)) {
            continue;
        }

        $tmpImage = sys_get_temp_dir() . DIRECTORY_SEPARATOR . 'cipop_ppt_img_' . uniqid('', true) . '.jpg';
        if (!convertImageToJpegForPpt($source, $tmpImage)) {
            continue;
        }

        $binary = @file_get_contents($tmpImage);
        if ($binary !== false) {
            $zip->addFromString($mediaPath, $binary);
        }
        $tmpImages[] = $tmpImage;
    ...
    foreach ($mediaSlideHeadings as $slideNum => $heading) {
        $slidePath = 'ppt/slides/slide' . $slideNum . '.xml';
        $slideXml = $zip->getFromName($slidePath);
        if ($slideXml === false) {
            continue;
        }
        $fixedSlide = pptxFixMediaSlideHeading($slideXml, $heading);
        if ($fixedSlide !== $slideXml) {
            $zip->addFromString($slidePath, $fixedSlide);
        }
    }
...
/**
 * Buat PDF lampiran amplifikasi Cipop MBG dari template PPT, fallback HTML jika konversi gagal.
 */
function createPdfCipopMbg(
    $outputPath,
    $imagePaths,
    $tanggalFormatted,
    $tanggalFormattedFirst,
    ...
    $pptTemplate = __DIR__ . '/template_ppt/format_Laporan_ampilifikasi.pptx';

    if (file_exists($pptTemplate)) {
        try {
            $tmpPptx = sys_get_temp_dir() . DIRECTORY_SEPARATOR . 'cipop_mbg_' . uniqid('', true) . '.pptx';
            createCipopMbgAmplifikasiPptxFromTemplate(
                $pptTemplate,
                $tmpPptx,
                $imagePaths,
                $tanggalFormattedFirst,
            ...
            if (convertPptxToPdfUsingPowerPoint($tmpPptx, $outputPath)
                || convertPptxToPdfUsingLibreOffice($tmpPptx, $outputPath)
            ) {
                @unlink($tmpPptx);
                return true;
            }

            error_log('Konversi PPT amplifikasi ke PDF gagal, fallback ke template HTML.');
            @unlink($tmpPptx);
        } catch (Exception $e) {
            error_log('createPdfCipopMbg via PPT: ' . $e->getMessage());
        }
    }
...
function handleLaporanMbgLengkap(
    ...
    $narasiAmplifikasi = buildNarasiAmplifikasiMbg($dataLink, $jumlahAkunperSheet, $jumlahLinkperSheet, $akunIndukData);
